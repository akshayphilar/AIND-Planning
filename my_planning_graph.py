...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
                ...
                s_nodes = a_node.precond_s_nodes()
                if s_nodes.issubset(self.s_levels[level]):
                    a_node.parents = a_node.precond_s_nodes()
                    a_node.children = a_node.effect_s_nodes()
    ...
    def add_literal_level(self, level):
        ...
        for a_node in list(self.a_levels[level-1]):
            for child in a_node.children:
                s_dict.setdefault(child, set())
                s_dict[child].add(a_node)
        self.s_levels.append(set())
        for s_node, parents in s_dict.items():
            s_node.parents = parents
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        #checking for effects that are added by one action while they are a neg precondition of the other
        if set(node_a1.action.effect_add).intersection(node_a2.action.precond_neg) or \
                set(node_a2.action.effect_add).intersection(node_a1.action.precond_neg):
            return True
        #checking for effects that are removed by one action while they are a precondition of the other
        elif set(node_a1.action.effect_rem).intersection(node_a2.action.precond_pos) or \
    ...
    def h_levelsum(self) -> int:
        ...
        for literal in self.problem.goal:
            for i in range(0, len(self.s_levels)):
                is_pos = True if literal.op[0] != '~' else False
                if PgNode_s(literal, is_pos) in list(self.s_levels[i]):
                    level_sum += i
                    break
        return level_sum
